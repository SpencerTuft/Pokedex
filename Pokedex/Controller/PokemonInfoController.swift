...
class PokemonInfoController: UIViewController {
    ...
    var pokemon: Pokemon? {
        didSet {
            ...
            // Could make this more dynamic for evolutions that don't hold this pattern
            if let evolutions = pokemon?.evolutions {
                if evolutions.count > 1 {
                    firstEvoImageView.image = evolutions[0].image
                    secondEvoImageView.image = evolutions[1].image
                } else {
                    firstEvoImageView.image = evolutions[0].image
                }
            }
    ...
    lazy var evolutionView: UIView = {
        ...
        evolutionLabel.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
        evolutionLabel.centerYAnchor.constraint(equalTo: view.centerYAnchor).isActive = true
